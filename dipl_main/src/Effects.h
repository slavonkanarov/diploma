...
public:
    Effects(){
        colorU8Index = 0;
        colorSign16Index = 0;
        // pullUpCollor(RED, GREEN, BLUE, BRIGHTNESS, 1, 0.5);
        colorLoop.start(0);
        way = true;
    }
    void refresh(){
    ...
    void pullUpCollor(  uint8_t red, uint8_t green, uint8_t blue, uint8_t brightness, 
                        uint8_t changing_block_size, float speed, Adafruit_NeoPixel matrix){
        ...
        if(colorLoop == 0){
            ...
            if(way == true){
                for(int i = 0; i < changing_block_size; i++){
                    matrix.setPixelColor(i+colorSign16Index, red, green, blue);   
                }
            } else {
                for(int i = 0; i < changing_block_size; i++){
                    matrix.setPixelColor(i+colorSign16Index, 0, 0, 0);
                }
            }
